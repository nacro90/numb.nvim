...
## Testing Guidelines
Run `scripts/check.sh` (or directly invoke `nvim --headless -u tests/init.lua -i NONE -n +"lua require('tests.run').run()" +qall`) to execute the automated headless regression tests; extend `tests/run.lua` whenever you fix a bug or add a new option. For exploratory work, still verify inside Neovim using `:lua require('numb').setup{centered_peeking=false}` and `:{number}` jumps. Temporary `require('numb.log').info(...)` calls are acceptable while debugging but must be removed before submission. Document any manual scenarios you covered in the PR description.
